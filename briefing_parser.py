...
def parse_briefing(docx_path):
    ...
    for text, is_bold in linhas:
        ...
        if m_foto and materia_atual is not None:
            ...
            # o ":" depois do "UNIR ESSA" inicial também é OPCIONAL (mesma
            # tolerância que o 08-08-26 deu ao 2º "essa"): o David às vezes
            # escreve "UNIR ESSA foto.png E ESSA outra.png" sem dois-pontos
            # nenhum (JM 22-09-26, união na CAPA). "unir essa" no começo da
            # linha da foto é marcador de união inequívoco (ninguém nomeia
            # arquivo assim), então tornar o ":" opcional não misfira; o caso
            # COM ":" fica idêntico (o ":" segue sendo consumido) — zero
            # regressão (todas as edições antigas usam "UNIR ESSA:").
            m_unir = re.match(r"unir\s+essa\b\s*:?\s*(.+)", conteudo, re.IGNORECASE)
            if m_unir:
                # separador entre as fotos: um conector ("e", "com", "+" ou
                # vírgula) + "essa" + ":" opcional. Ex. vistos no briefing real:
                # " E ESSA:", " COM ESSA", " + ESSA:", ", ESSA:", ", e ESSA:".
                # Duas tolerâncias que o David exercitou no JM 08-08-26:
                #  - o ":" depois de "essa" é OPCIONAL ("...11.jpg E ESSA FEIRA
                #    ...12.jpeg" sem dois-pontos no 2º); e
                #  - o conector pode ser "COM" ("...11.jpg COM ESSA ...12.jpeg"),
                #    não só "e"/"+".
                # Antes qualquer um desses fazia a união virar UMA foto só (o 2º
                # nome grudado no 1º). Guarda contra misfire: quando NÃO há ":",
                # exigimos um conector real antes ("e"/"com"/"+"/vírgula) e um
                # espaço depois — um nome de arquivo que por acaso contenha
                # "essa" não separa sozinho. O ramo "essa:" (com dois-pontos, sem
                # conector) fica idêntico ao comportamento antigo (zero regressão).
                partes = re.split(
                    r"\s*(?:"
                    r"(?:,\s*(?:\b(?:e|com)\b\s*)?|\+\s*|\b(?:e|com)\b\s*)essa\b\s*:?\s*"  # conector + essa (":" opcional)
                    r"|essa\s*:\s*"                                                        # essa: com ":" (sem conector)
                    r")",
                    m_unir.group(1), flags=re.IGNORECASE)
                nomes, focos_partes = [], []
                for parte in partes:
                    parte = parte.strip()
                    if not parte:
