...
def parse_briefing(docx_path):
    ...
    for text, is_bold in zip(paragraphs, bold_flags):
        ...
        if m_foto and materia_atual is not None:
            ...
            # marcação opcional de acabamento na linha da foto:
            #   (fundo desfocado) / (foto inteira) / (sem cortar) / (banner)
            # Quando presente, a foto entra INTEIRA (sem corte), com o vazio
            # preenchido por uma versão ampliada e desfocada dela mesma — em
            # vez de perder as laterais. A marca é removida antes de casar o
            # nome do arquivo.
            marca = re.search(
