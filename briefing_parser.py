...
def parse_briefing(docx_path):
    ...
    for text, is_bold in linhas:
        ...
        if m_foto and materia_atual is not None:
            ...
            # "Foto: ARQUIVO.jpg (foco: direita)" — aceita ':' ou espaço.
            m_foco_inline = re.search(
                r"[\(\[][^)\]]*foco[:\s]+"
                r"(esquerda|direita|centro|meio|cima|topo|baixo|base|fundo)"
                r"[^)\]]*[\)\]]", conteudo, re.IGNORECASE)
            if m_foco_inline:
                materia_atual["foco"] = m_foco_inline.group(1).lower()
                conteudo = (conteudo[:m_foco_inline.start()] + conteudo[m_foco_inline.end():]).strip()
