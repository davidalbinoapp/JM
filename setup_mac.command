...
# ---------------------------------------------------------------
# 5) Atalho "Núcleo Inteligente JM.app" (ícone da marca) — nasce sozinho aqui,
#    pra pessoa não precisar de nenhum passo extra depois de instalar.
# ---------------------------------------------------------------
if [ -f "Criar atalho (Mac).command" ]; then
  if bash "Criar atalho (Mac).command" --auto >/dev/null 2>&1; then
    ok "Atalho 'Núcleo Inteligente JM.app' criado na pasta acima da JM-main (arraste pro Dock se quiser)."
  else
    info "Não consegui criar o atalho automaticamente — dá pra criar depois com 'Criar atalho (Mac).command'."
  fi
fi
