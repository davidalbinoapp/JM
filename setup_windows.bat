...
REM ----------------------------------------------------------------
REM  5) Atalho "Nucleo Inteligente JM" na Area de Trabalho (icone da marca) -
REM     nasce sozinho aqui, sem nenhum passo extra depois de instalar.
REM     Aponta pro "Nucleo Inteligente JM (Windows).bat" e usa o icone .ico.
REM ----------------------------------------------------------------
echo.
powershell -NoProfile -Command "$s=New-Object -ComObject WScript.Shell; $l=$s.CreateShortcut((Join-Path ([Environment]::GetFolderPath('Desktop')) 'Nucleo Inteligente JM.lnk')); $l.TargetPath='%~dp0Nucleo Inteligente JM (Windows).bat'; $l.WorkingDirectory='%~dp0'; $l.IconLocation='%~dp0icone_montar_jm.ico'; $l.Description='Nucleo Inteligente JM - Jornal Mural'; $l.Save()" >nul 2>&1
if !errorlevel!==0 (
  echo [OK] Atalho "Nucleo Inteligente JM" criado na Area de Trabalho.
) else (
...
REM ================== subrotinas ==================
:ACHA_PYTHON
REM Define PYCMD com o Python REAL, sem sair da execucao atual.
REM NAO usamos "where": o Windows vem com um python.exe FALSO (alias da
REM Microsoft Store) que aparece no "where" mas so abre a Store, nao roda nada.
REM Por isso TESTAMOS RODANDO de verdade (-c "print(1)"): o alias falha nesse
REM teste e e' rejeitado, entao o instalador parte pra instalacao real do winget.
REM Ordem: launcher "py", "python" real no PATH, e as pastas onde o winget instala.
if defined PYCMD goto :eof
py -c "print(1)" >nul 2>&1 && ( set "PYCMD=py" & goto :eof )
python -c "print(1)" >nul 2>&1 && ( set "PYCMD=python" & goto :eof )
for /d %%D in ("%LOCALAPPDATA%\Programs\Python\Python3*") do if exist "%%D\python.exe" ("%%D\python.exe" -c "print(1)" >nul 2>&1 && set PYCMD="%%D\python.exe")
if defined PYCMD goto :eof
for /d %%D in ("%ProgramFiles%\Python3*") do if exist "%%D\python.exe" ("%%D\python.exe" -c "print(1)" >nul 2>&1 && set PYCMD="%%D\python.exe")
goto :eof
