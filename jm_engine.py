...
def _evitar_viuva_corpo(body_box, texto):
    """Prevenção de viúva na MASSA DE TEXTO do corpo: gruda as duas últimas
    palavras (espaço inseparável) pra a última linha não terminar com uma
    palavra sozinha. Mede na fonte regular do próprio box; se o par não couber
    numa linha, deixa quebrar natural (o _evitar_viuva já cuida disso). Não muda
    o nº de linhas (o par grudado ocupa a mesma última linha)."""
    runs = body_box.text_frame.paragraphs[0].runs
    pt = (runs[0].font.size / 12700) if (runs and runs[0].font.size) else 19
    return _evitar_viuva(texto, body_box.width, pt, negrito=False)
