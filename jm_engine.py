...
def _resolver_caminho_fonte(negrito):
    candidatos = _CANDIDATOS_ARIAL_BOLD if negrito else _CANDIDATOS_ARIAL_REGULAR
    for caminho in candidatos:
        if _os.path.isfile(caminho):
            return caminho
    return _os.path.join(FONTS_DIR, "LiberationSans-Bold.ttf" if negrito
...
def regenerate_final_page(pptx_path, soffice_script_path=None):
    ...
    Precisa do LibreOffice instalado (comando `soffice`) e do Poppler
    (comando `pdftoppm`) na máquina onde roda. Se qualquer um dos dois não
    estiver disponível, a função avisa e não faz nada — o resto do arquivo
    (capa, matérias, cartazes) já foi gerado normalmente antes desse passo."""
    import subprocess
    import shutil
    import tempfile
    ...
    if not soffice_cmd or not pdftoppm_cmd:
        faltando = []
        if not soffice_cmd:
            faltando.append("LibreOffice (comando 'soffice')")
        if not pdftoppm_cmd:
            faltando.append("Poppler (comando 'pdftoppm')")
        print(f"Aviso: página final não regenerada — falta instalar: {', '.join(faltando)}. "
              f"O restante do arquivo (capa, matérias, cartazes) foi gerado normalmente.")
        return
    ...
    subprocess.run(
        [pdftoppm_cmd, "-jpeg", "-r", "100", generated_pdf, os.path.join(tmp_dir, "pg")],
        check=True, capture_output=True,
    )
    thumb_paths = sorted(
