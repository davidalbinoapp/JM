...
def _foco_horizontal(foco):
    """Interpreta o override 'FOCO:' do briefing pro eixo horizontal."""
    if not foco:
        return None
    f = _normalize(foco)
    if f in ("esquerda", "esq", "left"):
        return "esquerda"
    if f in ("direita", "dir", "right"):
        return "direita"
    if f in ("centro", "center", "meio"):
        return "centro"
    return None


def _foco_vertical(foco):
    """Interpreta o override 'FOCO:' do briefing pro eixo vertical."""
    if not foco:
        return None
    f = _normalize(foco)
    if f in ("cima", "topo", "top"):
        return "cima"
    if f in ("baixo", "base", "bottom", "fundo"):
        return "baixo"
    if f in ("centro", "center", "meio"):
        return "centro"
    return None
...
def _offset_corte_horizontal(new_image_path, largura_total, new_w, foco=None):
    """Decide o x0 do corte horizontal enquadrando o ASSUNTO. Ordem:
    1) override manual do briefing (FOCO: esquerda/centro/direita), se houver;
    2) centro de importância = saliência (suprime multidão, acha o singular) +
       reforço dos rostos — resolve tanto a plateia que puxava o corte pro lado
       errado quanto a foto sem rosto com assunto fora do centro;
    3) fallback ao centro ponderado dos rostos / centro geométrico se a
    ...
    d = _foco_horizontal(foco)
    if d == "esquerda":
        return lo
    if d == "direita":
        return hi
    # "centro" NÃO força o centro geométrico da imagem — segue pro enquadramento
    # automático, que centraliza no ASSUNTO (rostos/saliência). Pedir "centro"
    # num trio contra um fundo largo deve centralizar o TRIO, não a foto.
...
def _offset_corte_vertical(new_image_path, altura_total, new_h, foco=None):
    """Decide o y0 do corte vertical. Override manual (FOCO: cima/centro/baixo)
    vem primeiro. Senão: se achar rostos, dá FOLGA acima da cabeça do rosto
    ...
    antigo — o vertical automático fica intocado, só ganha o override."""
    lo, hi = 0, altura_total - new_h
    d = _foco_vertical(foco)
    if d == "cima":
        return lo
    if d == "baixo":
        return hi
    # "centro" segue pro automático (folga acima das cabeças / assunto), em vez
    # de forçar o meio geométrico cego.
