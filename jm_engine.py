...
def _evitar_viuva_corpo(body_box, texto):
    ...
    de linhas; nunca deixa a última MAIOR que a penúltima; varre TODOS os k e
    pega o de melhor equilíbrio (não para no 1º platô — a diferença por k não é
    monotônica). Mede com uma pequena margem de APERTO (FATOR_APERTO_CORPO),
    ...
    for k in range(3, len(palavras)):
        ...
        if r[2] != n_linhas:       # mudaria o nº de linhas -> ignora esse k
            continue
        if r[1] > r[0]:            # a última passaria da penúltima -> ignora
            continue
