...
# o PowerPoint quebra o texto um pouco mais apertado que a medição por
# bbox do Pillow; medir o corpo com ~95% da largura antecipa isso, pra o
# equilíbrio das últimas linhas bater com o que aparece no arquivo final.
FATOR_APERTO_CORPO = 0.95
...
def _evitar_viuva_corpo(body_box, texto):
    ...
    que descem são grudadas com espaço inseparável. Regras: nunca aumenta o nº
    de linhas; varre TODOS os k e pega o de MELHOR equilíbrio — menor diferença
    de largura entre penúltima e última (não para no 1º platô, e não exige
    última <= penúltima, senão orfanaria a antepenúltima palavra, tipo
    "Parabéns" sozinho). Mede com uma pequena margem de APERTO (FATOR_APERTO_CORPO),
    porque o PowerPoint quebra um tico mais apertado que o cálculo por bbox —
    sem isso o motor acha que a última linha já está cheia quando o PowerPoint
    ainda a deixaria curta."""
    from PIL import ImageFont
    ...
    largura_px = (max(body_box.width - 2 * MARGEM_INTERNA_EMU, 12700) / 12700) * 4
    largura_px *= FATOR_APERTO_CORPO
    ...
    for k in range(3, len(palavras)):
        ...
        if r[2] != n_linhas:       # mudaria o nº de linhas -> ignora esse k
            continue
        # pega o menor |penúltima - última| GLOBAL (o mais equilibrado). NÃO
        # exige última <= penúltima: às vezes o ponto mais parelho deixa a
        # última um tico maior — e travar nisso orfanava a antepenúltima
        # palavra (ex.: "Parabéns" sozinho no fim da penúltima).
        if abs(r[0] - r[1]) < melhor_dif:   # melhor equilíbrio global
...
def resolve_foto(foto_arquivo, fotos_dir, target_ratio, warnings, contexto="",
    ...
    próprio, cai no `foco` compartilhado da matéria (compat)."""
    if isinstance(foto_arquivo, list):
        ...
        import hashlib
        assinatura = "||".join(foto_arquivo) + "##" + "|".join(str(f) for f in lista_focos)
...
def populate_double_slide(slide, story_top, story_bottom, fotos_dir, media_dir, warnings):
    ...
    for story, title_box, body_box, photo_box, tag_box, limite_inferior in [
        ...
        foto_path = resolve_foto(story.get("foto_arquivo"), fotos_dir,
                                  ...
                                  focos_unir=story.get("focos_unir"))
        if foto_path:
            _replace_picture(photo_box, foto_path,
...
def populate_single_slide(slide, materia, fotos_dir, media_dir, warnings):
    ...
    if photo_box:
        foto_path = resolve_foto(materia.get("foto_arquivo"), fotos_dir,
                                  ...
                                  focos_unir=materia.get("focos_unir"))
        if foto_path:
            _replace_picture(photo_box, foto_path,
                             foto_inteira=materia.get("foto_inteira", False),
...
def build_deck(template_path, paginas, fotos_dir, output_path, media_dir=None,
    ...
    if novas_materia_slides:
        ...
        _adicionar_logo_final(slides_originais[capa_idx])
    ...
    resto_ids = [sid for i, sid in enumerate(all_ids)
                 if i != capa_idx and i not in indices_a_remover]
