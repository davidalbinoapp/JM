...
def _evitar_viuva_corpo(body_box, texto):
    ...
    última linha ficar visualmente parelha com a penúltima — nem uma palavra
    solta, nem uma última linha vazia demais ("faço uma média"). As palavras que
    descem são grudadas com espaço inseparável. Regras de segurança: nunca
    aumenta o nº de linhas; nunca deixa a última linha MAIOR que a penúltima
    (para de descer no ponto de equilíbrio); se nem descer 2 couber numa linha,
    deixa a quebra natural."""
    from PIL import ImageFont
    ...
    while k < len(palavras):
        r = _sim(k)
        # para de descer se: não cabe, mudaria o nº de linhas, a última
        # passaria da penúltima, ou já não melhora o equilíbrio
        if r is None or r[2] != n_linhas or r[1] > r[0] or abs(r[0] - r[1]) >= melhor_dif:
            break
