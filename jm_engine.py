...
# Balanceamento da MASSA de corpo (a percepção editorial: olhar o bloco inteiro e
...
# — calibrado pro renderizador mais apertado (o PowerPoint).
FATOR_WRAP_RENDER = 0.955
# Fator de wrap SÓ pra decisão de encaixe do CORPO (quantas linhas cabem numa
# largura). O 0.955 acima foi calibrado pro TÍTULO (bold, que o PowerPoint fecha
# mais apertado) — no corpo ele aperta DEMAIS: conta 1 linha a mais que o render
# real, e o balanceador reduz o corpo pra 17/18 quando ele cabia a 19 alargando a
# caixa. Como a contagem (_contar_linhas_texto) já desconta a margem interna real
# de 0,1", aqui usamos 1.0 (sem aperto extra) — bate com o render medido (David
# 08-08-26: Desenv fecha em 7 linhas e cabe a 19; Kaizen fecha em 8 e precisa 18).
FATOR_WRAP_CORPO_FIT = 1.0
...
def _compor_ultima_linha_pp(body_box):
    ...
    toco solto QUANDO O POWERPOINT quebra. Espelha o ajuste manual de diagramação
    na capa: em vez de compensar no texto (espaço inseparável), encolhe um tico a
...
def _encaixar_corpo_preservando(title_box, body_box, texto_titulo, texto_corpo,
    ...
    """Encaixa título+corpo espelhando a PERCEPÇÃO editorial: olha a massa de texto
    ...
    ALTURA_LINHA = 1.25          # posicionamento do topo do corpo (conservador de propósito)
    # altura do CORPO na decisão de encaixe. Medido no render real (LibreOffice/
    # PowerPoint, 08-08-26): a linha do corpo fecha a **1,225** (não 1,25, que
    # derruba corpo que cabe; nem 1,2, que é otimista ~0,13cm e deixa vazar um
    # tico). Com 1,225 a estimativa do fim do bloco bate com o render quase exata
    # (Kaizen 14,04≈14,05; Visitas 28,92=28,92; Desenv 13,92≈13,93), então o motor
    # decide certo se cabe dentro da base da foto a 19pt ou se precisa reduzir.
    ALTURA_LINHA_FIT = 1.225
    PASSO = int(0.05 * 360000)
    ...
    plano = None
    transborda = False
    # tenta 19; só cai pra 18, 17,5 e por último 17 se NADA (título+largura) couber
    # (o David: 1ª redução é 18, no máx 17/17,5, nunca menos). O balanceador ALARGA
    # a caixa primeiro (larguras vão até a margem lateral) pra manter 19pt — só
    # reduz a fonte se nem no alargamento máximo couber dentro da base da foto.
    for pt_b in (pt_corpo, 18.0, 17.5, PT_PISO_CORPO):
        if pt_b > pt_corpo:
            continue
        ...
        for largura in larguras:
            ...
            n = _contar_linhas_texto(texto_corpo, int(largura * FATOR_WRAP_CORPO_FIT), pt_b, negrito=False)
            altura_larg[largura] = int(n * pt_b * 12700 * ALTURA_LINHA_FIT)
        candidatos = []
    ...
    if plano is None:
        ...
        n = _contar_linhas_texto(texto_corpo, int(largura * FATOR_WRAP_CORPO_FIT), PT_PISO_CORPO, negrito=False)
        alt = int(n * PT_PISO_CORPO * 12700 * ALTURA_LINHA_FIT)
    ...
    # DECISÃO AUTORITATIVA de fonte/largura do corpo (regra do David: o corpo NÃO
    # pode passar da base da máscara da foto, mas PODE encostar nela). Mede o fim
    # REAL do bloco (topo já aplicado por _reposicionar, régua fiel 1.0 + 1,225) e
    # escolhe a MAIOR fonte que cabe — preferindo a largura ATUAL (a que o leque
    # escolheu por distribuição) e, só se precisar, ALARGANDO até o máximo (o que o
    # David faz à mão) antes de reduzir a fonte. Isso corrige o caso em que o leque
    # (estimativa otimista do topo do corpo) já tinha baixado a fonte: aqui a fonte
    # é reconsiderada do 19 pra baixo. TOL deixa o texto ENCOSTAR na base sem
    # reduzir à toa (o Kaizen do David fecha 8 linhas terminando exatamente na base).
    TOL = int(0.12 * 360000)      # ~1mm + descendente: deixa o texto ENCOSTAR na base
    lim = limite_inferior_emu + TOL
    w_atual = body_box.width
    w_max = int(largura_max_corpo_emu) if largura_max_corpo_emu else w_atual
    w_max = max(w_max, w_atual)
    def _fim_real_corpo(w, pt):
        n = _contar_linhas_texto(body_box.text_frame.text,
                                 int(w * FATOR_WRAP_CORPO_FIT), pt, negrito=False)
        return body_box.top + int(n * pt * 12700 * ALTURA_LINHA_FIT)
    def _menor_largura_que_cabe(pt):
        # ALARGA só o necessário: menor largura em [w_atual, w_max] onde cabe (o
        # David abre "um pouco" a caixa, não até a borda). Passo de 0,4cm.
        passo = int(0.4 * 360000)
        w = w_atual
        while w < w_max:
            if _fim_real_corpo(w, pt) <= lim:
                return w
            w += passo
        return w_max if _fim_real_corpo(w_max, pt) <= lim else None
    escolha = None
    for pt in (pt_corpo, 18.0, 17.5, float(PT_PISO_CORPO)):
        if pt > pt_corpo:
            continue
        w = _menor_largura_que_cabe(pt)
        if w is not None:
            escolha = (pt, w); break
    if escolha is None:               # nem no piso couber: fica no piso, mais largo
        escolha = (float(PT_PISO_CORPO), w_max)
    pt_b, w_final = escolha
    if w_final != body_box.width:
        body_box.width = w_final
    for paragraph in body_box.text_frame.paragraphs:
        for r in paragraph.runs:
            r.font.size = Pt(pt_b)
    _set_run_text_keep_format(
        body_box.text_frame, _glue_viuva_corpo(texto_corpo, body_box.width, pt_b, FATOR_WRAP_RENDER))
    transborda = _fim_real_corpo(body_box.width, pt_b) > lim
    if _DEBUG_MASSA:
        print(f"[MASSA-FIT] {contexto[:26]!r} pt_b_final={pt_b:g} W={body_box.width/360000:.2f} "
              f"fim={_fim_real_corpo(body_box.width, pt_b)/360000:.2f} limite={limite_inferior_emu/360000:.2f}")
    if transborda and warnings is not None:
        warnings.append(f"{contexto}: muito texto — corpo mantido em {pt_b:g}pt e "
...
# ---------------------------------------------------------------------------
# Fluxo dinâmico dos boxes na página única (compensação de transbordo)
# ---------------------------------------------------------------------------
# Em vez de deixar as caixas em posição FIXA do template (o que prendia o corpo
# num espaço apertado e transbordava), o motor reflui o bloco de boxes A PARTIR
# da base real do corpo, na largura cheia da coluna, com cada caixa do tamanho
# do seu conteúdo — espelhando o ajuste manual de diagramação. O corpo ganha
# mais folga vertical (reservamos só o
# necessário pros boxes) e nada fica cravado onde não cabe.
GAP_CORPO_BOXES_CM = 0.45   # respiro entre a base do corpo e o topo do bloco de boxes
GAP_TOPO_BOXES_CM = 0.1     # do divisor de topo até o 1º elemento (header/caixa)
GAP_HEADER_BOX_CM = 0.15    # do header até a 1ª caixa
GAP_ENTRE_BOXES_CM = 0.35   # entre uma caixa e a próxima
_INSET_V_EMU = 2 * 45720    # margens internas verticais padrão da caixa (0,05" x2)
_ALTURA_LINHA = 1.25
_PT_HEADER_BOX = 18.0       # fonte do título da seção de boxes ("Período ...:"): 18pt
                            # negrito — mesmo tamanho das caixas, em bold (cara de
                            # título). Valor de referência da diagramação manual.
_CAL_CORPO_FLUXO = 1.0      # fator de calibração da altura do corpo p/ posicionar os
                            # boxes. 1.0 = respiro de referência (o estimador é
                            # conservador; abaixar aperta o bloco contra o corpo)


def _altura_texto_emu(texto, largura_emu, pt, negrito=False):
    """Altura (EMU) que um texto ocupa numa caixa de dada largura/fonte —
    nº de linhas reais x altura de linha + margens internas. Permite dimensionar
    a caixa ao conteúdo em vez de deixar uma altura fixa sobredimensionada."""
    n = _contar_linhas_texto(texto, largura_emu, pt, negrito=negrito)
    return int(n * pt * 12700 * _ALTURA_LINHA) + _INSET_V_EMU


def _pt_do_box(placeholders, default=14.0):
    """Tamanho (pt) usado pra DIMENSIONAR/espaçar as caixas no fluxo. Não é a
    fonte visível (as caixas herdam 18 do molde e o autosize ajusta a altura no
    render) — é a medida compacta que reproduz o encaixe de referência: caixas
    encostadas, deixando o corpo em fonte cheia (título 40 / corpo 19) sem
    encolher. Lido do run do placeholder; 14 quando o run não traz sz."""
...
def _boxes_efetivos(boxes_data, n_ph):
    """Lista final (label, texto) a renderizar: se há mais boxes que
    placeholders, junta o excedente no último (espelha _render_boxes)."""
    if n_ph <= 0:
        return []
    if len(boxes_data) <= n_ph:
        return [(b["label"], b["texto"]) for b in boxes_data]
    principais = [(b["label"], b["texto"]) for b in boxes_data[:n_ph]]
    resto = boxes_data[n_ph:]
    extra = " ".join(((b["label"] + ": ") if b["label"] else "") + b["texto"] for b in resto)
    lab, txt = principais[-1]
    principais[-1] = (lab, (txt + " " + extra).strip())
    return principais


def _reserva_altura_boxes(boxes_ef, header_text, col_width, pt_box):
    """Altura vertical (EMU) que o bloco de boxes vai consumir — pra dar TETO ao
    corpo (reserva) antes de encaixá-lo, deixando espaço garantido pras caixas."""
    total = int(GAP_TOPO_BOXES_CM * 360000)
    if header_text:
        total += _altura_texto_emu(header_text, col_width, _PT_HEADER_BOX, negrito=True)
        total += int(GAP_HEADER_BOX_CM * 360000)
    for i, (lab, txt) in enumerate(boxes_ef):
        s = ((lab + ": ") if lab else "") + txt
        total += _altura_texto_emu(s, col_width, pt_box)
        if i < len(boxes_ef) - 1:
            total += int(GAP_ENTRE_BOXES_CM * 360000)
    return total


def _fluxo_boxes_unica(slide, body_box, placeholders, linhas, boxes_ef, header_text,
                       pt_box, col_left, col_width, base_util, warnings, contexto=""):
    """Posiciona header + caixas EM FLUXO logo abaixo da base real do corpo, na
    largura cheia da coluna, cada caixa do tamanho do conteúdo. Reaproveita os
    placeholders/divisores do template (reposicionando-os) e cria o header como
    caixa própria. Chamado DEPOIS de encaixar o corpo e a foto."""
    from pptx.util import Pt
    from pptx.dml.color import RGBColor
    n = len(boxes_ef)
    if n == 0:
        return
    col_left, col_width = int(col_left), int(col_width)

    # base real do corpo (medida pelo texto, não pela altura autosize do shape).
    # O contador de linhas roda ~conservador (aperta a régua pra NUNCA transbordar),
    # então superestima a altura do corpo em ~15%. Pra encostar o bloco de boxes no
    # corpo (sem respiro fantasma), aplica um fator de calibração render-vs-estimador
    # — mas mantendo folga: mesmo calibrado, a base fica ABAIXO da última linha real,
    # nunca sobrepondo o corpo.
    corpo_txt = body_box.text_frame.text
    pt_corpo = _fonte_pt_do_corpo(body_box)
    corpo_bot = (body_box.top or 0) + int(
        _altura_texto_emu(corpo_txt, body_box.width, pt_corpo) * _CAL_CORPO_FLUXO)
    y = corpo_bot + int(GAP_CORPO_BOXES_CM * 360000)

    # preenche e recorta os placeholders (remove os que sobram)
    for extra in placeholders[n:]:
        extra._element.getparent().remove(extra._element)
    usados = placeholders[:n]
    for (lab, txt), ph in zip(boxes_ef, usados):
        _preencher_box_textbox(ph, lab, txt)
        ph.text_frame.word_wrap = True

    # divisores: precisamos de n (1 no topo do bloco + 1 antes de cada caixa
    # seguinte). Reposiciona os existentes; remove o excedente.
    dividers = sorted(linhas, key=lambda s: s.top or 0)
    for extra in dividers[n:]:
        extra._element.getparent().remove(extra._element)
    dividers = dividers[:n]

    def _por_divisor(dv, top):
        dv.left, dv.width, dv.top, dv.height = col_left, col_width, int(top), 0

    di = 0
    if di < len(dividers):
        _por_divisor(dividers[di], y); di += 1
    y += int(GAP_TOPO_BOXES_CM * 360000)

    # header (linha "Período ...:") como caixa própria acima das caixas — 18pt
    # negrito (mesmo tamanho das caixas, em bold = título), como no ajuste manual de referência
    if header_text:
        h_h = _altura_texto_emu(header_text, col_width, _PT_HEADER_BOX, negrito=True)
        tb = slide.shapes.add_textbox(col_left, int(y), col_width, int(h_h))
        tf = tb.text_frame; tf.word_wrap = True
        r = tf.paragraphs[0].add_run(); r.text = header_text
        r.font.bold = True; r.font.size = Pt(_PT_HEADER_BOX); r.font.color.rgb = RGBColor(0, 0, 0)
        cr = body_box.text_frame.paragraphs[0].runs
        if cr and cr[0].font.name:
            r.font.name = cr[0].font.name
        y += h_h + int(GAP_HEADER_BOX_CM * 360000)

    # caixas em fluxo
    for i, ph in enumerate(usados):
        if i > 0 and di < len(dividers):
            _por_divisor(dividers[di], y - int(GAP_ENTRE_BOXES_CM * 360000 / 2)); di += 1
        lab, txt = boxes_ef[i]
        s = ((lab + ": ") if lab else "") + txt
        h = _altura_texto_emu(s, col_width, pt_box)
        ph.left, ph.width, ph.top, ph.height = col_left, col_width, int(y), int(h)
        y += h + int(GAP_ENTRE_BOXES_CM * 360000)

    fim = y - int(GAP_ENTRE_BOXES_CM * 360000)
    if base_util and fim > base_util + int(0.4 * 360000) and warnings is not None:
        warnings.append(f"{contexto}: o bloco de boxes passou da base da foto — "
                        f"considere encurtar o texto das caixas.")
...
def _parse_zoom(foco):
    """Lê um ZOOM explícito no FOCO do briefing e devolve o FATOR (>1) ou None.
    Aceita: 'zoom' sozinho (fator padrão ZOOM_FOCO_2D = +18%), 'zoom 15%' ou
    'zoom 15' (percentual → 1.15) e 'zoom 1.2' (fator direto). Serve pra AMPLIAR
    a foto um tico e criar folga de reenquadramento no eixo que o corte 'cover'
    não deixa mexer — ex.: foto cortada em cima/baixo que o cliente quer empurrar
    pra esquerda pra centralizar as pessoas. Teto de 2.0 pra não estourar a
    resolução/pixelar."""
    if not foco:
        return None
    s = unicodedata.normalize("NFKD", str(foco)).encode("ascii", "ignore").decode().lower()
    m = re.search(r"zoom\s*([0-9]+(?:[.,][0-9]+)?)?\s*(%)?", s)
    if not m:
        return None
    if not m.group(1):
        return ZOOM_FOCO_2D
    val = float(m.group(1).replace(",", "."))
    if m.group(2) or val > 3:      # percentual (tem % ou número "grande": 15 → 1.15)
        z = 1.0 + val / 100.0
    else:                          # fator direto (ex.: 1.2)
        z = val
    return max(1.0, min(z, 2.0))
...
def _replace_picture(picture_shape, new_image_path, corte_diagonal=True,
    ...
    if foto_inteira:
        ...
        zoom = _parse_zoom(foco)
        if dois_eixos or zoom:
            # foco 2D (opt-in) OU zoom explícito: amplia um tico pra dar folga nos
            # dois eixos e posiciona cada eixo pelo seu foco. O zoom explícito
            # também DESTRAVA um foco lateral numa foto cortada em cima/baixo (e
            # vice-versa) — a ampliação cria a folga que o cover não dava, então
            # o "esquerda"/"direita" passa a ter efeito. Sem foco/zoom, o corte é
            # byte-a-byte o de sempre (ramo de baixo).
            im = _crop_zoom_2d(new_image_path, target_ratio, foco,
                               zoom=zoom or ZOOM_FOCO_2D)
        else:
...
def _mosaic_layouts(n):
    ...
    if n == 3:
        return [
            # 1 alta à esquerda + 2 à direita (layout de referência: a foto deitada
...
def _compose_capa_split_photo(photo_paths, target_ratio, focos=None):
    """União de fotos na CAPA em corte RETO (não diagonal), preservando a ORDEM
    do briefing. A caixa de foto da capa é ALTA (retrato); o corte diagonal de
    fatias verticais do `_compose_merged_photo` espremeria cada foto numa tira
    fina (corta metade das pessoas) — ruim aqui. Este compositor divide a caixa
    em DUAS metades e escolhe, POR PONDERAÇÃO de orientação, entre:

      - EMPILHADO  (2 faixas largas, foto 1 em cima / foto 2 embaixo) — o
        "MODELO 2" do David, que casa com fotos DEITADAS numa caixa alta;
      - LADO A LADO (2 colunas altas, foto 1 à esquerda / foto 2 à direita) —
        o "MODELO 1", melhor quando as fotos são EM PÉ.

    Diferente do mosaico (que PERMUTA foto→célula), aqui a atribuição é fixa
    (foto i → célula i), então a ordem do briefing é respeitada (drone em cima,
    equipe embaixo no JM 22-09-26). Cada foto ocupa metade e é enquadrada pelo
    ASSUNTO (mesmo corte inteligente: saliência + rostos), respeitando o FOCO
    por foto. Filete branco fino entre as metades, como nas demais uniões. Para
    n != 2 (raro numa capa) cai no mosaico, que trata 3/4 fotos."""
    import math
    n = len(photo_paths)
    if n != 2:
        return _compose_mosaic_photo(photo_paths, target_ratio, focos)
    if focos is None:
        focos = [None] * n
    altura_ref = 1000
    largura_total = int(altura_ref * target_ratio)
    ...
    # dois candidatos, SEM permutar (ordem preservada): empilhado vs lado a lado
    empilhado = [(0, 0, 1.0, 0.5), (0, 0.5, 1.0, 0.5)]
    lado_a_lado = [(0, 0, 0.5, 1.0), (0.5, 0, 0.5, 1.0)]

    def _custo_layout(layout):
        # desencaixe de orientação (mesma métrica log do mosaico): quão longe a
        # proporção de cada foto está da célula que vai ocupar. Empatou → o
        # empilhado ganha (ver <= abaixo), que é o padrão do David pra capa.
        total = 0.0
        for i, (_x, _y, wf, hf) in enumerate(layout):
            cell_ratio = (wf * largura_total) / (hf * altura_ref)
            total += abs(math.log(ratios[i]) - math.log(cell_ratio))
        return total

    layout = empilhado if _custo_layout(empilhado) <= _custo_layout(lado_a_lado) else lado_a_lado

    canvas = Image.new("RGB", (largura_total, altura_ref), (255, 255, 255))
    m = divisor_px // 2
    for i, path in enumerate(photo_paths):
        xf, yf, wf, hf = layout[i]
        x0, y0 = int(round(xf * largura_total)), int(round(yf * altura_ref))
        x1, y1 = int(round((xf + wf) * largura_total)), int(round((yf + hf) * altura_ref))
        # filete branco só nas bordas INTERNAS (entre as metades), não em volta
        ix0 = x0 + (m if x0 > 0 else 0)
        iy0 = y0 + (m if y0 > 0 else 0)
        ix1 = x1 - (m if x1 < largura_total else 0)
        iy1 = y1 - (m if y1 < altura_ref else 0)
...
# formatos que o Pillow abre nativamente — o resto (RAW de câmera: .ARW, .NEF,
# .CR2/.CR3, HEIC...) precisa ser convertido antes, senão o Image.open estoura.
_EXTS_PILLOW = {".jpg", ".jpeg", ".png", ".webp", ".bmp", ".gif", ".tif", ".tiff"}
_PADRAO_CONV = re.compile(r"^_conv_[0-9a-f]+\.jpg$", re.IGNORECASE)


def _garantir_legivel(path, fotos_dir, warnings, contexto=""):
    """Garante que a foto casada é um arquivo que o Pillow abre. Se for um RAW de
    câmera (ou outro formato exótico que o Pillow não decodifica), converte pra
    JPG uma vez via `sips` (nativo do macOS) e devolve o caminho do JPG cacheado
    (`_conv_<hash>.jpg` na própria pasta, no mesmo esquema dos `_merged_`; a
    faxina do build apaga depois). Fotos em formato normal passam direto, sem
    reprocessar. Se a conversão falhar ou não houver conversor (ex.: Windows sem
    sips), avisa e devolve None — a matéria sai sem a foto, mas o build NÃO cai
    por causa de uma foto no formato errado."""
    if not path:
        return None
    ext = os.path.splitext(path)[1].lower()
    if ext in _EXTS_PILLOW:
        return path  # caminho rápido: nada a converter

    # chave determinística por arquivo+mtime: re-rodar reaproveita o cache, e
    # trocar a foto (mesmo nome, mtime novo) força reconversão
    try:
        mtime = int(os.path.getmtime(path))
    except OSError:
        mtime = 0
    import hashlib
    chave = hashlib.md5(f"{os.path.realpath(path)}::{mtime}".encode("utf-8")).hexdigest()[:12]
    destino = os.path.join(fotos_dir, f"_conv_{chave}.jpg")
    if os.path.exists(destino):
        return destino  # cache quente
    ...
    if not sips:
        if warnings is not None:
            warnings.append(f"{contexto}: foto '{os.path.basename(path)}' em formato "
                            f"não suportado ({ext}) e sem conversor disponível — matéria "
                            f"sai sem a foto.")
        return None
    try:
        _subprocess.run([sips, "-s", "format", "jpeg", path, "--out", destino],
                        check=True, capture_output=True, timeout=120)
    except (_subprocess.CalledProcessError, _subprocess.TimeoutExpired, OSError):
        if warnings is not None:
            warnings.append(f"{contexto}: falha ao converter '{os.path.basename(path)}' "
                            f"({ext}) — matéria sai sem a foto.")
        return None
    if not os.path.exists(destino):
        if warnings is not None:
            warnings.append(f"{contexto}: conversão de '{os.path.basename(path)}' não gerou "
                            f"saída — matéria sai sem a foto.")
        return None
    return destino


def resolve_foto(foto_arquivo, fotos_dir, target_ratio, warnings, contexto="",
                 foco=None, focos_unir=None, mosaico=False, uniao_capa=False):
    ...
    `uniao_capa=True` (só a capa passa) compõe a união num corte RETO adequado à
    caixa alta da capa — empilhado ou lado a lado, ver `_compose_capa_split_photo`
    — em vez do corte diagonal de fatias verticais. O `mosaico` explícito do
    briefing continua tendo prioridade sobre o `uniao_capa`."""
    if isinstance(foto_arquivo, list):
        ...
        for nome in foto_arquivo:
            ...
            if not p:
                ...
                return None
            p = _garantir_legivel(p, fotos_dir, warnings, f"{contexto} (foto '{nome}')")
            if not p:
                return None  # RAW ilegível numa parte da união: já avisou; aborta a união
            paths.append(p)
        ...
        if mosaico:
            ...
            composta = _compose_capa_split_photo(paths, target_ratio, focos=lista_focos)
        else:
        ...
        import hashlib
        assinatura = ("mosaico::" if mosaico else "capa::" if uniao_capa else "") + "||".join(foto_arquivo) + "##" + "|".join(str(f) for f in lista_focos)
        ...
        if not p:
            ...
            return None
        return _garantir_legivel(p, fotos_dir, warnings, contexto)
...
# Dupla — REGRA do David: o corpo NÃO pode vazar por cima da foto NEM passar da
# BASE DA MÁSCARA DA FOTO. O limite de cada corpo é a base da SUA foto (estrito).
# O balanceador ALARGA a caixa até esta margem lateral pra manter 19pt; só reduz
# a fonte (18 → 17,5 → 17) se nem alargando couber dentro da base da foto.
MARGEM_LATERAL_DUPLA_CM = 0.5   # quão perto da borda a caixa pode alargar
# Segurança: o corpo de baixo nunca passa desta margem da base da prancheta
# (a base da foto de baixo, que é o limite real, costuma ser mais alta que isso).
MARGEM_BASE_DUPLA_CM = 0.3

# QR code na CAPA: diferente da única (placeholder quadrado no molde) e da dupla
# (QR ao lado do título), a capa recebe o QR na MESMA faixa vertical (rente à
# base da foto), escolhendo a COLUNA conforme o espaço:
#  - se sobra espaço livre ABAIXO da massa de texto, o QR vai na coluna de texto
#    (canto inferior esquerdo da prancheta) — solução preferida do David (2.pptx);
#  - senão (corpo longo que desce até perto da base), cai SOBRE a foto, no canto
#    inferior esquerdo dela (1.pptx, a base que ele montou à mão).
# Valores (5.15cm quadrado, ~0.75cm de respiro das bordas da foto) replicam as
# duas bases manuais do JM 08-08-26. Ver populate_capa.
LADO_QR_CAPA_CM = 5.15
INSET_QR_CAPA_CM = 0.75
# Respiro mínimo (cm) entre o fim do corpo e o topo do QR pra considerar que
# "cabe" na coluna de texto; abaixo disso, o QR vai pra foto.
FOLGA_QR_TEXTO_CM = 0.5
...
def populate_capa(slide, materia, fotos_dir, media_dir, warnings):
    ...
    if photo_box:
        # geometria da foto ANTES de trocar (o _replace_picture remove e recria o
        # shape na mesma posição/tamanho → a referência photo_box fica velha, mas
        # estes 4 valores continuam valendo pro QR abaixo).
        foto_left, foto_top = photo_box.left, photo_box.top
        foto_width, foto_height = photo_box.width, photo_box.height
        ...
        foto_path = resolve_foto(materia.get("foto_arquivo"), fotos_dir, ratio, warnings, "CAPA",
                                 ...
                                 mosaico=materia.get("mosaico"), uniao_capa=True)
        if foto_path:
        ...
        # QR na capa. A capa tem QR quando a matéria da capa casou com um arquivo
        # de QR (mesmo casamento por título das demais páginas). Ao contrário da
        # única/dupla, o molde da capa não traz placeholder de QR — o motor DESENHA
        # o QR. Faixa vertical fixa (rente à base da foto); a COLUNA depende do
        # espaço: se há folga abaixo do corpo, vai na coluna de texto (preferido);
        # senão, sobre a foto. Ver constantes LADO/INSET/FOLGA_QR_*.
        qr_path = materia.get("qr_path")
        if qr_path:
            lado = Emu(int(LADO_QR_CAPA_CM * 360000))
            inset = Emu(int(INSET_QR_CAPA_CM * 360000))
            qr_top = foto_top + foto_height - lado - inset
            qr_left = foto_left + inset   # padrão/fallback: sobre a foto (1.pptx)
            destino = "foto"
            if body_box is not None:
                # mede onde o corpo REALMENTE termina (régua de render) e vê se o
                # QR cabe abaixo dele, na coluna de texto (2.pptx).
                pt_corpo = _fonte_pt_do_corpo(body_box)
                texto_corpo = body_box.text_frame.text
                corpo_bottom = body_box.top + _altura_texto_emu(
                    texto_corpo, body_box.width, pt_corpo)
                folga = Emu(int(FOLGA_QR_TEXTO_CM * 360000))
                if corpo_bottom + folga <= qr_top:
                    qr_left = body_box.left   # há espaço livre: QR na coluna de texto
                    destino = "coluna de texto"
            slide.shapes.add_picture(qr_path, qr_left, qr_top, lado, lado)
            if _DEBUG_MASSA:
                print(f"[QR CAPA] destino={destino} left_cm={qr_left/360000:.2f}")
    else:
        warnings.append("CAPA: não encontrei a foto principal pra substituir.")
        if materia.get("qr_path"):
            warnings.append("CAPA: tem QR, mas sem a foto de capa não dá pra posicioná-lo.")
...
def populate_double_slide(slide, story_top, story_bottom, fotos_dir, media_dir, warnings):
    ...
    # limite ESTRITO = base da máscara da foto de cada matéria (o corpo não passa
    # disso). O balanceador alarga a caixa pra caber a 19pt; se não couber, reduz a
    # fonte. Nada de tolerância pra baixo — o texto não pode vazar da foto.
    limite_top_story = photo_top.top + photo_top.height
    limite_bottom_story = min(photo_bottom.top + photo_bottom.height,
                               slide_height - Emu(int(MARGEM_BASE_DUPLA_CM * 360000)))
    ...
    for story, title_box, body_box, photo_box, tag_box, limite_inferior in [
        ...
        # nas 2 linhas que o layout reserva (foi o ajuste manual de referência no
        ...
        slide_w = slide.part.package.presentation_part.presentation.slide_width
        largura_max = slide_w - body_box.left - Emu(int(MARGEM_LATERAL_DUPLA_CM * 360000))
...
def populate_single_slide(slide, materia, fotos_dir, media_dir, warnings):
    ...
    header_text = " ".join(materia.get("boxes_intro") or []).strip() or None
    linhas_slide = [s for s in slide.shapes if s.shape_type == 9]
    placeholders = textboxes[2:]
    # FLUXO dinâmico dos boxes (compensa transbordo): exige boxes no briefing,
    # placeholders no molde e a foto pra definir a coluna. Senão, cai no caminho
    # antigo (posição fixa / remoção).
    usar_fluxo = bool(boxes_data and placeholders and photo_box is not None
                      and photo_box.left is not None and photo_box.left > body_box.left)
    boxes_ef = pt_box = col_left = col_width = base_util = None
    if title_box and body_box:
        slide_h = slide.part.package.presentation_part.presentation.slide_height
        limite_inferior = slide_h - Emu(int(1.0 * 360000))
        largura_max = None
        if usar_fluxo:
            # coluna cheia (até a borda da foto − folga) e base útil = base da foto
            col_left = body_box.left
            col_width = photo_box.left - body_box.left - Emu(int(FOLGA_CORPO_FOTO_CM * 360000))
            col_width = max(col_width, Emu(int(6.0 * 360000)))
            base_util = min(limite_inferior, (photo_box.top or 0) + (photo_box.height or 0))
            pt_box = _pt_do_box(placeholders)
            boxes_ef = _boxes_efetivos(boxes_data, len(placeholders))
            reserva = _reserva_altura_boxes(boxes_ef, header_text, col_width, pt_box)
            # TETO do corpo = base útil − o que os boxes vão consumir. Assim o corpo
            # ganha toda a folga possível e o bloco de boxes reflui ABAIXO dele
            # (em vez de o corpo ficar preso acima de um divisor cravado no molde).
            limite_inferior = max(base_util - reserva - Emu(int(GAP_CORPO_BOXES_CM * 360000)),
                                  (title_box.top or 0) + Emu(int(2.0 * 360000)))
            # corpo na largura cheia da coluna: menos linhas ⇒ menos altura ⇒ menos
            # risco de transbordo. Só no caminho de boxes (não mexe nas outras únicas).
            largura_max = col_width
            body_box.width = col_width
        else:
            # comportamento antigo (sem boxes, ou layout sem foto/coluna). TETO DE
            # LARGURA: corpo pode alargar até a coluna, nunca por cima da foto.
            ...
            if photo_box is not None and photo_box.left is not None and photo_box.left > body_box.left:
                ...
                col_max = max(col_max, Emu(int(6.0 * 360000)))
                largura_max = col_max
    ...
    # boxes do briefing ("ABRIR/CRIAR BOX ...:"): fluxo dinâmico (compensa
    # transbordo, refluindo abaixo do corpo) quando aplicável; senão preenche em
    # posição fixa (layout sem foto/coluna) ou remove as caixas extras e linhas.
    if usar_fluxo:
        _fluxo_boxes_unica(slide, body_box, placeholders, linhas_slide, boxes_ef,
                           header_text, pt_box, col_left, col_width, base_util,
...
def _limpar_merged(fotos_dir):
    """Apaga os JPEGs intermediários que o motor grava na pasta da semana:
    fotos unidas (`_merged_*.jpg`) e RAWs convertidos pra JPG (`_conv_*.jpg`).
    Depois do .pptx salvo, os bytes já ficam embutidos no arquivo — esses
    intermediários não servem mais. Rodado no início do build (limpa restos de
    execuções anteriores) e no fim (limpa os desta execução), pra não acumular
    na pasta do cliente."""
    try:
        for nome in os.listdir(fotos_dir):
            if _PADRAO_MERGED.match(nome) or _PADRAO_CONV.match(nome):
                try:
