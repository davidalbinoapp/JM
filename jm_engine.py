...
def _offset_corte_horizontal(new_image_path, largura_total, new_w):
    """Decide o x0 do corte horizontal. Se achar rostos, centra a janela no
    centro ponderado deles e garante conter a caixa dos rostos quando ela cabe;
    senão, corta pelo centro (comportamento antigo)."""
    ...
    if (rx1 - rx0) <= new_w:                       # dá pra conter todos os rostos
        x0 = min(max(x0, int(rx1 - new_w)), int(rx0))
    x0 = max(lo, min(hi, x0))
    # rede de segurança: se uma BORDA do corte cai no meio de um rosto, escorrega
    # pro vão mais próximo — cada rosto fica inteiro dentro ou inteiro fora,
    # nunca pela metade (é o "espaço neutro" que absorve o ajuste).
    return _snap_fora_dos_rostos(x0, new_w, rostos, lo, hi)
...
def _offset_corte_vertical(new_image_path, altura_total, new_h):
    """Decide o y0 do corte vertical. Se achar rostos, dá FOLGA acima da cabeça
    do rosto mais alto (não decepa cabeça/cabelo/boné) e joga o excedente pra
    BASE — desde que a foto tenha área embaixo. Se não houver rosto, usa o viés
    fixo pro topo (VIES_CORTE_VERTICAL), o comportamento antigo."""
...
def _replace_picture(picture_shape, new_image_path, corte_diagonal=True,
    ...
    if foto_inteira:
        ...
        if src_ratio > target_ratio:
            ...
            # em vez do corte central cego, desloca pra manter as pessoas
            # enquadradas (cai no centro se não houver rosto/detector)
            x0 = _offset_corte_horizontal(new_image_path, w, new_w)
            im = im.crop((x0, 0, x0 + new_w, h))
        else:
            new_h = int(w / target_ratio)
            # dá folga acima das cabeças e joga o excedente pra base (cai no
            # viés fixo pro topo se não houver rosto)
            y0 = _offset_corte_vertical(new_image_path, h, new_h)
...
def _compose_merged_photo(photo_paths, target_ratio):
    ...
    for path in photo_paths:
        ...
        if src_ratio > sub_ratio:
            ...
            # enquadra pelas pessoas (cai no centro se não houver rosto), igual
            # ao corte das fotos normais — antes era corte central cego
            x0 = _offset_corte_horizontal(path, w, new_w)
            im = im.crop((x0, 0, x0 + new_w, h))
        else:
            new_h = int(w / sub_ratio)
            y0 = _offset_corte_vertical(path, h, new_h)  # folga acima das cabeças
            im = im.crop((0, y0, w, y0 + new_h))
