...
def _marcador(texto, cor="verde"):
    """Destaca um texto com fundo colorido (marca-texto) no terminal. Se a
    saída não for um terminal (ex.: redirecionada pra arquivo), devolve o
    texto puro, sem os códigos de cor."""
    if not sys.stdout.isatty():
        return texto
    fundos = {"verde": "42", "amarelo": "43"}
    codigo = fundos.get(cor, "42")
    return f"\033[30;{codigo}m {texto} \033[0m"  # texto preto sobre fundo colorido


# AGA em ASCII art (bloco), pra dar as boas-vindas quando o comando roda.
_AGA_ASCII = r"""
    █████    ██████    █████
   ██   ██  ██        ██   ██
   ███████  ██   ███  ███████
...
def _banner():
    """Mostra o 'AGA' em ASCII art, em laranja no terminal (sem cor se a saída
    for redirecionada pra arquivo). Puramente estético."""
    laranja = "\033[38;5;208m" if sys.stdout.isatty() else ""
    fim = "\033[0m" if sys.stdout.isatty() else ""
    # centraliza o subtítulo sob o desenho do AGA (largura calculada, então
    # continua alinhado mesmo se o ASCII art mudar)
    linhas = _AGA_ASCII.strip("\n").split("\n")
    largura = max(len(l) for l in linhas)
    sub = "J O R N A L   M U R A L"
    pad = max(0, (largura - len(sub)) // 2)
    print(laranja + _AGA_ASCII + fim)
    print(laranja + " " * pad + sub + fim + "\n")
...
def main():
    ...
    if todos:
        print(f"\n{_marcador('Finalizamos, mas com avisos:', 'amarelo')} Arquivo gerado em: {output_path}")
        print(f"\n{len(todos)} aviso(s):")
        for a in todos:
            print(" -", a)
    else:
        print(f"\n{_marcador('Finalizamos com sucesso!')} Arquivo gerado em: {output_path}")
        print("Nenhum aviso! Tudo casou certinho <3")
