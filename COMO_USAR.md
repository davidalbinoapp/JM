...
- **`Núcleo Inteligente JM.command`** (Mac) / **`Nucleo Inteligente JM
  (Windows).bat`** (Windows) — o atalho de **dois cliques**: abre uma janela pra
  você escolher a pasta da semana e já monta o JM (sem digitar caminho). É o
  jeito mais fácil de rodar no dia a dia. Use o que combina com o seu computador.
  (No Windows o nome vai sem acento — "Nucleo" — de propósito, pra não dar
  problema de codificação no atalho.)
- **`auto_jm.py`** — o motor do atalho. Recebe **só a pasta da semana** e monta
  tudo (acha o briefing e casa os QR codes sozinho). É o que o atalho
...
- **No Mac:** `Núcleo Inteligente JM.command`
- **No Windows:** `Nucleo Inteligente JM (Windows).bat`
...
- **Mac, 1ª vez:** o macOS pode dizer "desenvolvedor não identificado" —
  clique com o **botão direito** no `Núcleo Inteligente JM.command` → **Abrir** →
  **Abrir**. Depois disso, dois cliques normais funcionam sempre.
- **Atalho com ícone da marca — nasce sozinho na instalação.** Quando você
  ...
  já cria o atalho **Núcleo Inteligente JM** com o ícone da marca, sem nenhum
  passo extra:
  - **Mac:** um **`Núcleo Inteligente JM.app`** na pasta "JM SYSTEM" (ao lado das
    pastas de semana). Arraste pro Dock se quiser. O ícone fica dentro do app,
    então viaja junto se você copiar o app.
  - **Windows:** um atalho **"Nucleo Inteligente JM"** na Área de Trabalho.
...
- **Aceita foto em formato RAW da câmera** (`.ARW` da Sony, `.CR2`/`.CR3` da
  Canon, `.NEF`, etc.): o motor converte pra JPG sozinho e usa normalmente. Se
  por algum motivo não der pra converter, a matéria sai sem aquela foto **com um
  aviso** — antes, uma foto RAW derrubava a montagem inteira.
...
(`baixo 30% direita 25%` = `direita 25% baixo 30%`).

### Ampliar a foto de propósito (`zoom`)

Às vezes a foto está "presa": ela é aparada só num eixo (ex.: cortada em
cima/baixo), então pedir `esquerda`/`direita` **não tem efeito** — não há folga
lateral pra deslocar. Pra esses casos (ou sempre que quiser **aproximar** um
pouco a foto pra as pessoas ficarem mais presentes/centralizadas), use o
**`zoom`**:

```
Foto: NOME DO ARQUIVO.jpg (foco: esquerda 40% zoom 15%)
```

- **`zoom 15%`** amplia a foto 15% — e essa ampliação **cria a folga** que
  faltava, destravando o `esquerda`/`direita` (ou `cima`/`baixo`) pra você
  reposicionar. Ou seja: o `zoom` controla **o quanto amplia**; as direções
  controlam **pra onde** vai. Dá pra combinar os dois eixos:
  `(foco: esquerda 40% baixo 10% zoom 15%)`.
- Também aceita `zoom 1.2` (fator direto) ou só `zoom` (padrão, +18%). O teto é
  2× (pra não pixelar).
- Pode escrever o zoom **junto** do foco, como acima, **ou num marcador
  separado** — tanto faz: `(foco: esquerda 40%) (zoom: 15%)` funciona igual.
- Sem `zoom`, nada muda: o corte continua exatamente o de sempre.

O foco 2D automático (dois focos juntos) continua existindo e aplica um zoom
discreto sozinho; o `zoom` acima é quando **você** quer decidir a ampliação.
...
comece a linha com **`ABRIR BOX ...:`** ou **`CRIAR UM BOX ...:`** (os dois
verbos funcionam, com qualquer texto no meio, ex.: `CRIAR UM BOX COM ESSAS
INFORMAÇÕES E INCLUIR NESTE SLIDE:`) e coloque **cada box em um parágrafo**, no
formato **`Rótulo: conteúdo`**:
...
- **Linha de título da seção** (uma frase que termina em `:` mas **sem** conteúdo
  depois, ex.: `Período de reconfiguração dos rádios comunicadores:`) vira um
  **cabeçalho** acima das caixas, em negrito — não é tratada como um box.
- A diretiva pode vir **antes ou depois da linha `Foto:`** — tanto faz, o motor
  reconhece nas duas posições.
- O layout pronto do cliente comporta **2 boxes**. Se você mandar mais que
  ...
  manter até 2.
- **Não transborda mais:** quando há box, o motor recompõe o slide sozinho —
  alarga o texto de abertura pra ocupar a coluna cheia, encaixa cada caixa no
  tamanho do conteúdo e desce o bloco de boxes na medida certa, fechando acima
  da foto. Antes, texto+box comprido passava do limite e você tinha que ajustar
  à mão.

## Várias unidades no mesmo JM (MG, QZ, LM, CDS, GO)

Por padrão o JM é da unidade **MG** (Minas Gerais). Às vezes a mesma edição
atende **outras unidades** da AGA também — **QZ** (Queiroz), **LM** (Lamego),
**CDS** (Córrego do Sítio), **GO** (Goiás). Quase sempre as matérias se
**repetem** entre as unidades; muda uma página ou outra. Em vez de duplicar o
JM inteiro, você marca no briefing **para onde cada página vai**.

**Como marcar no briefing:** antes de cada página, ponha uma linha
**`Unidades:`** com as siglas entre parênteses:

```
Unidades: (MG) (QZ) (LM) (CDS)
Página única:
Editoria: Excelência Operacional
Conheça os PGRs da AGA
...

Unidades: (CDS)
Página única >> Incluir
Editoria: Segurança
Conheça os novos membros da CIPAMin de CDS
...
```

- **Página sem `Unidades:`** = vai pra **MG** (o padrão). Um briefing sem
  nenhuma tag funciona exatamente como sempre.
- A tag também pode vir na própria linha da página
  (`Página única >> ... (MG) (QZ) (LM)`) — tanto faz.
- A logo de fechamento entra em **cada página marcada com
  `/ FECHA COM LOGO DA AGA /`** — no multi-unidade há uma última página por
  unidade, cada uma com a sua logo.

**O que aparece no PowerPoint:**

- **Selo em cada slide** — uma pilha de **círculos coloridos** com a sigla da
  unidade, na lateral direita **fora do slide** (uma cor por unidade). Serve de
  guia visual pra quem monta; como fica fora da área do slide, **não sai na
  impressão nem no PDF**.
- **Um gabarito por unidade no fim** — a página final (com as miniaturas)
  passa a ter **um gabarito por conjunto de páginas**. Unidades que recebem
  exatamente as mesmas páginas **compartilham um gabarito** (com todas as
  siglas no rodapé). No exemplo acima, MG/QZ/LM ganham um gabarito (com os
  PGRs) e CDS ganha o seu (com a CIPAMin no lugar).

Nada muda na diagramação — é só a indicação de unidade que entra.
