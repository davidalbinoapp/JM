...
## Enquadramento inteligente de pessoas (automático)

Quando uma foto é mais larga que o espaço dela na arte, o motor precisa cortar
as laterais. Antes ele cortava sempre pelo **centro** — o que às vezes deixava
uma pessoa **pela metade** se ela estivesse fora do meio da foto. Agora o motor
**detecta os rostos** e desloca o corte pra **manter as pessoas enquadradas**,
sozinho. Não precisa marcar nada no briefing.

- Em foto **sem pessoas** (prédio, gráfico, banner), nada muda: usa o corte
  central de sempre.
- Depende da biblioteca **OpenCV**, instalada automaticamente pelos
  instaladores (`setup_mac.command` / `setup_windows.bat`). É **opcional**: se
  ela não estiver presente, o JM é gerado normalmente com o corte central —
  só sem o ajuste automático.
- O modelo de detecção (arquivo `models/face_detection_yunet.onnx`, ~230 KB)
  vai junto no repositório, como as tags e as fontes.
